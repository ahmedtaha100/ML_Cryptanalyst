...
class FeatureSelector:
    """Feature selection and dimensionality reduction for side-channel analysis."""
    ...
    def select_poi_sost(self, traces: np.ndarray, labels: np.ndarray,
        ...
        The SOST method identifies points where power consumption varies
        most between different key hypotheses, indicating high leakage.

        Parameters
        ----------
        traces : ndarray of shape (n_traces, n_samples)
            Power traces to analyze
        labels : ndarray of shape (n_traces,)
            Known labels (e.g., key bytes) for each trace
        num_poi : int, default=1000
            Number of points to select

        Returns
        -------
        indices : ndarray of shape (num_poi,)
            Indices of selected points
        selected_traces : ndarray of shape (n_traces, num_poi)
            Traces with only selected points

        References
        ----------
        .. [1] Standaert et al., "Using Subspace-Based Template Attacks 
           to Compare and Combine Power and Electromagnetic Information 
           Leakages", CHES 2008
        """
    ...
    def select_poi_ttest(self, traces: np.ndarray, labels: np.ndarray,
                         num_poi: int = 1000) -> Tuple[np.ndarray, np.ndarray]:
        """Select POI using Welch's t-test (for 2 classes only)."""
        unique_labels = np.unique(labels)
    ...
    def select_poi_mutual_information(self, traces: np.ndarray, labels: np.ndarray,
                                      num_poi: int = 1000) -> Tuple[np.ndarray, np.ndarray]:
        """Select POI using mutual information."""
    ...
    def transform(self, traces: np.ndarray) -> np.ndarray:
        ...
        if self.selected_indices is None:
            raise ValueError("No feature selection has been fitted")
        return traces[:, self.selected_indices]
    def apply_pca(self, traces: np.ndarray, n_components: Union[int, float] = 0.95, batch_size: Optional[int] = None) -> np.ndarray:
    ...
    def cross_validate_poi_selection(self, traces: np.ndarray, labels: np.ndarray,
        ...
        """Cross-validate different numbers of POI to avoid overfitting.
        ...
        num_poi_list : list of int
            Different numbers of POI to evaluate
        cv : int, default=5
        ...
        -------
        dict
            Results with mean and std accuracy for each num_poi
        """
